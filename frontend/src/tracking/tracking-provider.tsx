...
export function TrackingProvider({ children }: TrackingProviderProps) {
  ...
  const startTracking = useCallback(
    async ({ tripId, accessToken: startAccessToken, currentUserId }: StartTrackingInput) => {
      if (status !== 'idle') {
        return
      }
      setStatus('starting')
      ...
      try {
        ...
        const uploader = new SessionUploader({
          ...
          onSnapshotChange: setUploaderSnapshot,
        ...
        setActiveSession({ sessionId, startedAt, tripId })
        setStatus('recording')
        void refreshQueueStats(sessionId)
      } catch (startError) {
    ...
    [handleFix, handleTerminated, refreshQueueStats, status],
  )
