...
class ScheduledJob(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            'length(trim(timezone)) > 0', name='ck_jobs_timezone_not_blank'
        ),
    )
    ...
    updated_by: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey('users.id', ondelete='SET NULL', name='fk_jobs_updated_by'),
        nullable=True,
    )
...
class JobExecution(Base):
    ...
    __table_args__ = (
        ...
        Index(
            'uq_job_executions_active_job',
            'job_key',
            unique=True,
            postgresql_where=text("status IN ('QUEUED', 'RUNNING')"),
        ),
        CheckConstraint(
            "trigger IN ('SCHEDULED', 'STARTUP', 'MANUAL')",
    ...
    job_key: Mapped[str] = mapped_column(
        String(100),
        ForeignKey('jobs.key', ondelete='CASCADE', name='fk_job_executions_job_key'),
        nullable=False,
    )
    ...
    requested_by: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey(
            'users.id', ondelete='SET NULL', name='fk_job_executions_requested_by'
        ),
        nullable=True,
    )
