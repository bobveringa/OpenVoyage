...
class MediaCleanupService:
    ...
    def cleanup_orphans(self, *, cutoff: datetime, batch_size: int = 100) -> MediaCleanupResult:
        totals = MediaCleanupResult()
        failed_ids = set()
        while True:
            statement = select(Media.id).where(
                    Media.created_at < cutoff,
                    ~exists(select(UserProfile.user_id).where(UserProfile.profile_picture_media_id == Media.id)),
            ...
            candidates = self.db.scalars(statement.order_by(Media.created_at).limit(batch_size)).all()
            if not candidates:
                return totals
            for media_id in candidates:
                scanned = totals.scanned + 1
                try:
                    media = self.db.scalar(select(Media).where(Media.id == media_id).with_for_update())
                    if media is None or not self._is_orphan(media, cutoff):
                        self.db.rollback()
                        totals = MediaCleanupResult(**{**totals.__dict__, 'scanned': scanned})
                        continue
                    ...
                    logger.exception('media_cleanup_item_failed', extra={'media_id': str(media_id)})
                    totals = MediaCleanupResult(**{**totals.__dict__, 'failed_media': totals.failed_media + 1, 'scanned': scanned})

    def _is_orphan(self, media: Media, cutoff: datetime) -> bool:
        if media.created_at >= cutoff:
            return False
        return not any((
            self.db.scalar(select(exists().where(UserProfile.profile_picture_media_id == media.id))),
    ...
    def _remove_files(self, media: Media) -> tuple[int, int]:
        ...
        deleted = missing = 0
        for stored_path in filter(None, (media.storage_path, media.thumbnail_storage_path)):
